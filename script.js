...
document.getElementById('buy-btn').addEventListener('click', function() {
    ...
    const currency = document.querySelector('.payment-btn.active').getAttribute('data-currency');
    const rateInfo = RATES[currency];
    const wlfgAmountText = wlfgAmountEl.textContent;
    ...
    if (amount < rateInfo.min) {
        // Error is already shown by updateWlfgAmount, but we can shake the form
        document.querySelector('.presale-form').style.animation = 'shake 0.5s';
        setTimeout(() => { document.querySelector('.presale-form').style.animation = ''; }, 500);
        return;
    }
    ...
    document.getElementById('modal-receive-amount').innerHTML = `${totalWlfg.toLocaleString()} WLFG<br><small style="color: #ffd700;">(${baseWlfg.toLocaleString()} base + ${bonusWlfg.toLocaleString()} bonus)</small>`;
...
document.getElementById('claim-airdrop-btn').addEventListener('click', function() {
    showReferralModal('🎁 Automatic Airdrop', `
        <p>Airdrop is automatic! Your rewards will be distributed automatically after the presale ends.</p>
        ...
        <div style="background: var(--bg-secondary); padding: 1rem; border-radius: 8px; margin: 1rem 0;">
            <p style="margin: 0; color: var(--text-secondary);"><strong>Reward Details:</strong></p>
            <p style="margin: 0.5rem 0 0 0; color: var(--text-secondary);">• 2,500 WLFG per successful referral</p>
